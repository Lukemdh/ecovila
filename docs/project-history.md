...
- **2026-08-27 — ADR-107 follow-up (B-37).** Live CRM showed `Total neverificat` on every calendar card. Root cause measured against prod: the bound-link read sent every visible reservation id in one PostgREST `in.(...)` URL filter — 600 ids answered 200, 900 ids (~33KB) answered 400 — so the read threw and the fail-closed display fired everywhere. Both bound-link reads now chunk at 200 ids per request. Also corrected the display: the unverified state no longer badges every card or relabels totals; it stays on the write path (repricing refused) and in the cancel/move warnings. `npm test` → 427 Node + 202 Deno; token `?v=2026082703`; `dist/tophost` regenerated; frontend-only, needs a TopHost re-upload.
- **2026-08-27 — ADR-108 reservation dialog regrouped.** Owner-requested UX pass on the reservation popup under a no-new-text/no-new-buttons constraint, planned with Gemini. Deleted the stale repricing hint; moved payment status and the total into a header row; reordered fields to follow a phone call and paired name/phone; put the two rare operations side by side under one divider (full width when opened, via `:has()` with a stacking fallback); moved the `data-refund-full-override` checkbox next to the cancellation controls and onto the existing `.crm-check` row, fixing a layout bug where `.crm-field`'s grid rendered it above its own label; gave the danger zone its own tinted panel. No JS, no behaviour and no `data-*` hook changed. Verified by measuring the real markup against the real stylesheet in a browser. `npm test` → 427 Node + 202 Deno; token `?v=2026082704`; frontend-only.
- **2026-08-27 — ADR-109 calendar loading overlay.** The calendar now shows a spinner while it loads. Armed on a 250 ms delay so the frequent realtime reloads never strobe it, except on first paint where it shows at once; only the newest load clears it, so an out-of-order reload cannot dismiss a live spinner. Needed a `.crm-calendar-viewport` wrapper because `.crm-calendar` is the scroll container. `pointer-events: none` so a reload never swallows a click; `aria-busy` instead of visible text; reduced-motion slows rather than freezes it. `npm test` → 428 Node + 202 Deno; frontend-only.
- **2026-08-27 — ADR-110 MIA links stop looking like card payments.** `plata.html`'s pre-payment screen is shown for both rails and carried the maib/Mastercard/Visa marks plus a `Plătește cu cardul` button even for a MIA link — an instant bank transfer. Brand block and button now follow the rail, reusing the existing `payLink.miaButton` string (no new copy); the i18n key is swapped along with the text so a language switch cannot restore the card wording. Also fixed the failed-start path, which restored the label only for the card rail. `npm test` → 430 Node + 202 Deno; frontend-only.
- **2026-08-27 (session close) — ADR-105/106/107 fully live; ADR-108/109/110 uploaded.** TopHost upload verified on the live host by CONTENT, not just by the `?v=` stamp: the stale repricing line is gone (0 hits), the spinner markup and calendar viewport are present, the regrouped dialog header and `.crm-check` refund row are present, the MIA brand block is on `plata.html`, the served `crm-dashboard.js` carries the loader delay and no longer carries the per-card `Total neverificat` badge, and the served `js/supabase.js` carries the B-37 id chunking. Migration ledger aligned; backend untouched since the function deploy; working tree clean and pushed (2c74b5d). Outstanding: nothing has been paid through a payment link and no cross-type move has been billed with real money yet; B-34/B-35/B-36 remain open.
- **2026-08-31 — ADR-111 guest dossier, flag markers and returning-guest alert (OFF-PLAN, not deployed).** Owner asked for persistent notes about customers (past ones included), an email when a noted customer books again, a colour + exclamation marker on the calendar, and the same signal in both halves of Situația zilnică. Planned with Codex and Gemini proposing independently and then attacking the merged plan; their findings changed the design in several load-bearing ways. New `public.guest_notes` (append-only, immutable bodies, no DELETE grant for any role, forced authorship and forced archive metadata via triggers), `public.guest_flag_exclusions` seeded with the office phone and mailbox, and the `public.guest_flag_markers` view (`security_invoker`). Identity is phone OR email, per the owner. The alert is a NEW per-minute cron function `send-guest-flag-alerts` rather than a hook in `create-reservation`: that keeps Resend out of the synchronous checkout path (where a hang can strand a committed booking), catches staff-created bookings (a direct table insert), and is the only shape that can actually retry. Recipient-missing, failed-attempt and stable-owner guards each close a specific hole found in review. Frontend: badge + composed inset ring on calendar cards, an `Istoric client` panel in the reservation dialog, an inline flagged pill on both Daily lists, an opt-in "save to the guest's history" checkbox on checkout, and a flag strip in the add-reservation form. Fixed B-38 (a live data-loss bug: opening a past reservation from search and saving wiped party size, child ages and notes) and logged B-39 (`review_request` missing from the repo's event-type constraint). Verified by browser measurement against the real stylesheet; `npm test` → 440 Node + 214 Deno; token `?v=2026083101`; `dist/tophost` regenerated. **Nothing applied, deployed or scheduled** — awaiting owner sign-off.
- **2026-08-31 (deploy) — ADR-111 backend LIVE; B-39 repaired in production.** Preflight found more than the expected drift: `review_request` was absent from the LIVE `notification_events` CHECK as well as the repo's, `notification_events` held zero such rows, and the review-request cron had run 8,160 times reporting success while sending nothing — the function catches each guest's failure into `console.error` and still answers 200. 552 booking groups went un-emailed between 2026-06-24 and 2026-08-30. Owner chose to let review requests resume from the next evening's checkouts and to have a catch-up proposal worked out separately rather than mailing the 552. Applied `20260831120000` + `20260831123000`, repaired history, redeployed all 31 Edge Functions, scheduled `ecovila-guest-flag-alerts`, and confirmed it fires. Post-deploy probing then caught a real gap: Supabase's default privileges had already given service_role `DELETE` on the new table, which a narrower GRANT does not take back — `20260831130000` revokes it, so only `postgres` can hard-delete a guest note. Trigger invariants probed live inside a self-rolling-back block (forged author overwritten; `body` and `guest_phone` updates both 42501) with nothing persisted. Merged and pushed to main. Remaining: the owner's manual TopHost upload of `?v=2026083101`.
- **2026-08-31 — B-39 catch-up built, dry-run verified, one live test sent (no bulk send yet).** Owner chose: last 30 days, one email per person, ~4 days of pacing, complainers excluded, live test first. Added the disposable `backfill-review-requests` function (dry-run / test / send modes, never scheduled) plus `selection.ts` and 5 Deno tests. It reuses the live `selectReviewRequestGroups` rather than restating eligibility, so it can only mail people the real flow would have mailed. The dry run reported 144 against an independent SQL count of 145 — the gap was a genuine defect, not arithmetic: complaints were excluded per booking, so someone who complained about one stay could still be asked to review another. Now excluded per person, with a regression test; both counts agree at 144. One live test email delivered to the owner's address, rendered from a real recipient but recording nothing, so that guest still receives their own copy. `npm test` → 440 Node + 219 Deno. **Nothing sent to guests.**
- **2026-08-31 — B-39 catch-up run 1 sent (36), remaining 108 paced by a bounded cron.** Owner approved the test email and asked for the rest to run without hand-holding. Run 1: 36 sent, 0 failed, verified in the database (36 `review_request` rows, all `sent`, all tagged `backfill`). Added `20260831140000_review_backfill_cron.sql` — `'0 10 1-4 9 *'`, i.e. 10:00 UTC on 1-4 September and then never again. Three runs clear the backlog and the fourth is a deliberate spare that should return `{"sent":0}`. Bounded on purpose: an open-ended daily job would keep calling a finished function forever. No double-send risk against the live 18:30 flow — both dedup on the booking group's stable owner id, and the windows do not overlap. **To delete when done: the function, the `ecovila-review-backfill` job, and its migration.**
- **2026-08-31 — full QA pass on the session's work (Gemini frontend + Codex modules + live probing).** Six defects found and fixed, all against already-deployed code. Most important: the earlier deploy probe ran as `postgres`, which BYPASSES RLS, so the policies had never actually been tested — re-probed with real staff JWT claims and confirmed Angela cannot forge authorship (her claimed `created_by_role='diana'` stored as `angela`) and cannot archive (0 rows, because RLS denies an UPDATE by row count rather than by raising, which an exception-based probe would have misread as success). Fixed B-40 (backfill treated `reserved`/`failed` review events as delivered and silently retired those guests; complainers resolved only inside the window; unpaginated reads truncating silently — now fail loudly) and B-41 (dossier severity pills signalled state by colour alone, focus outline 1.27:1 → 5.35:1). Removed `_preview/` which an over-broad `git add -A` had committed. Verified the 36 sent emails: 37 events, 37 distinct people, 0 duplicates, 0 complainers — and the 37th proved B-39's fix end-to-end, being the live flow's first send in ten weeks. `npm test` → 440 Node + 220 Deno. Accepted-and-documented rather than fixed: the at-least-once send window, mutable-email dedup, staggered-checkout group splitting (0 of 285 groups affected), and the 100-row previous-stay bound.
