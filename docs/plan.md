...
- **2026-06-03 — OFF-PLAN four owner-reported fixes.** B-16 arrival reminders gated to 10:00 Europe/Chisinau (new `_shared/reminders.ts` + `tests/reminders.test.ts`); B-17 `confirmare.js` recovers `id`/`manage` from the `ecovila_pending_reservation` localStorage record after the Maib redirect (fixes the post-card-payment "Rezervarea nu a fost găsită" page) without weakening the manage-token requirement; B-18 the managed confirmation view shows the cash hold panel/timer only while pending, the card box only for card, and the cash office-only refund note for cash; B-19 `.cf-panels` wrapper removes the confirmed↔manage desktop layout gap. Added ADR-019/ADR-020. No plan step status changed. Verified 188 Node + 41 Deno tests, `deno lint`/`deno check`/`deno fmt --check`, and a live two-column confirmation-page preview.
- **2026-06-03 — OFF-PLAN cookie banner redesign.** Redesigned the consent banner (10 public pages, ro/ru/en) as a card with icon, title/subtitle, full-width "Accept toate", and a "Setări cookie-uri | Doar esențiale" row; moved the category checkboxes + custom save behind the settings toggle. Consent logic unchanged (added only a `[data-cookie-settings]` reveal handler in `main.js`); rewrote `.cookie-banner*` CSS; added/updated `cookie.*` translation keys. Added ADR-021. No plan step status changed. Verified 188 Node + 41 Deno tests and a live browser preview of all consent paths.
...
- **2026-06-08 — OFF-PLAN CRM delete confirmation and rolling calendar fix.** Replaced
  ...
  smoke.
- **2026-06-11 — OFF-PLAN payment-flow audit, Critical fixes, and production deploy.** Found and fixed B-23 (client-controlled reservation price; added `_shared/pricingGuard.ts` server-side recomputation in `create-reservation`, 409 on mismatch, `_shared/pricing.js` kept byte-identical to `js/pricing.js` by `tests/pricing-guard.test.mjs`), B-24 (anon direct INSERT on `reservations`; migration `20260611120000_revoke_public_reservation_insert.sql`), B-25 (MAIB callback amount reconciliation), plus B-26 (test sold-out scaffolding removed from `booking.js`), B-27 (holiday fetch no longer date-range-filtered), B-28 (pricing-load failure blocks checkout), B-29 (MAIB session reuse requires exact amount match), B-30 (`.or()` filter injection in `maib-refund`), B-31 (CRM cookie `Secure`). Deployed to production: migration applied individually via the management API because local/remote migration histories are drifted (ADR-023 — never plain `supabase db push` on this project); `create-reservation`, `maib-callback`, `maib-create-payment`, `maib-refund` redeployed and verified live (tampered total → 409, anon insert → 42501, valid booking created and cleaned up). No plan step status changed. Updated README, production-readiness-audit, security, bugs, decisions, conventions, project-structure, project-history, this plan, and the root `bugs.md`. Verified `npm test` (205 Node + 48 Deno), `deno check`, and live production probes. Outstanding: upload `dist/tophost/`, MAIB sandbox end-to-end payment, rotate the access token used for the deploy.
- **2026-08-27 — OFF-PLAN standalone payment links in CRM (ADR-106).** Implemented standalone, single-use payment links in the CRM and guest page `plata.html` (independent of reservations; no booking/room inventory changes, no `Purchase` tracking). Diana mints links from a Diana-only CRM tab ("Linkuri de plată") with amount in integer MDL, rail (MIA or card MAIB), optional label (max 120 chars), and optional expiry (`Fără expirare` / 1h / 3h / 8h), copying `https://ecovila.md/plata.html?p=<uuid>`. Provider sessions are minted lazily on "Plătește" (`payment-link-public` action `start`) with claim-before-mint attempt rows (`claim_payment_link_attempt`) keyed to MAIB `orderId`. Callbacks and client polling re-read MAIB authoritatively (`GET /v2/checkouts/{id}` for card, `GET /v2/mia/payments?orderId=` for MIA) and settle atomically via `settle_payment_link_attempt` RPC; captured money always wins, settling late captures as paid under `manual_review`. Staff portal refunds are recorded (*Marchează ca restituit*) without moving money, ensuring Finance net figures remain accurate. Finance folds net link income (`paid_amount − refunded_amount`) into `onlineTotal` and `commercialTotal` in `Încasări` mode only, with an Online sub-line and dedicated "Plăți din linkuri" section, preserving reservation-only metrics (`occupiedNights`, `roomTypeTotals`, `paidBookings`, and average booking value). Added `plata.html` (`noindex`, no analytics), `js/plata.js`, `css/payment-link.css`, `admin/js/crm-payment-links.js`, `_shared/paymentLinks.ts`, `payment-link-admin`, `payment-link-public`, and migration `20260826120000_payment_links.sql` (Diana-only SELECT RLS, no client write policies, service-role-only `SECURITY INVOKER` RPCs with `search_path = ''`). Logged B-33 (Finance UTC midnight vs Europe/Chisinau binning). No plan step status changed; CURRENT STEP remains Step 17. Reviewed and updated: `README.md` (key pages, test counts), `project-overview.md` (capabilities, architecture, data flows), `project-structure.md` (files, table, flows), `project-history.md` (session entry), `production-readiness-audit.md` (readiness addendum, test counts), `security.md` (RLS, RPC, bearer capability model, staff gating), `bugs.md` (logged B-33), `conventions.md` (SQL migration & RPC standards), `plan.md` (session log); `decisions.md` (ADR-106 entry added). Verified: `npm run test:node` (412 passed) and `npm run test:deno` (186 passed).
- **2026-08-27 — OFF-PLAN ADR-107 documentation Definition of Done (docs only,
  nothing committed/deployed).** Reconciled `PLAN-FINAL.md` against the complete tracked
  diff and all four untracked implementation files, then documented the built
  accommodation-move/bound-difference behavior, not the earlier plan where they differ
  (notably nullable legacy `booking_group_id` binding). Added ADR-107; logged B-34
  (paid add-guests difference is quoted again in Daily) and B-35 (a paid Daily reprice
  appears as uncollected income on the original `paid_at`); recorded the migration/RPC,
  effective-total, Finance/Daily, cancellation/manual-refund, monotonic-refund,
  Realtime, review, asset-token and rollout facts. Reviewed and updated all ten required
  DoD documents.
- **2026-08-27 — OFF-PLAN ADR-107 post-QA documentation update (docs only, nothing committed/deployed).**
  Reconciled docs with the final two-lens QA round and the six code fixes: (1) documented the
  `prevent_repricing_with_accommodation_difference` DB trigger as the true authoritative enforcement
  point against concurrent Daily edits; (2) documented `move_reservation_accommodation` attempt locking
  refusing replacement billing when a provider attempt is live; (3) documented the optimistic room-type
  re-reading in `_shared/reservationChanges.ts` before inserting pending add-guests changes along with its
  residual window; (4) documented strict `paid_amount` verification (missing/invalid `paid_amount` fails closed,
  routed to pessimistic copy + staff alert, never falling back to requested `amount`); (5) documented fetching
  cancellation bound-link refunds strictly by `reservation_id` (so ungrouped bookings are not dropped); and
