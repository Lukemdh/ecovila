...
describe('EcoVila cancellation policy alignment', () => {
  it('explains the online cancellation window and cash office reimbursement on the page', () => {
    ...
    assert.match(html, /cel puțin 20 zile calendaristice/i);
  ...
  it('treats the exact twentieth calendar day and fresh bookings as refundable in EcoVila time', () => {
    ...
    assert.equal(isRefundEligible('2026-06-05', noonUtcOnMay16), true);
    assert.equal(isRefundEligible('2026-06-04', noonUtcOnMay16), false);
    assert.equal(
  ...
  it('keeps translations aligned with the 20-day refundable and non-refundable messages', () => {
